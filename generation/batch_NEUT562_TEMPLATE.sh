...
shifter -V ${PWD}:/output --entrypoint PrepareNEUT -m ${E_MONO} -i ${OUTFILE} -o ${OUTFILE/.root/_prep.root}
shifter -V ${PWD}:/output --entrypoint nuisflat -f GenericVectors -i NEUT:${OUTFILE/.root/_prep.root} -o ${OUTFILE/.root/_NUISFLAT.root} -q "nuisflat_SaveSignalFlags=false"

echo "Complete"

## Copy back the important files
cp ${TEMPDIR}/${OUTFILE/.root/_prep.root} ${OUTDIR}/.
